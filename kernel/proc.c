...
found:
  ...
  // Create kernal pagetable for the process
  p->kernel_pagetable = proc_create_kernel_pagetable(0);
  if (p->kernel_pagetable == 0) {
    freeproc(p);
    release(&p->lock);
    return 0;
  }
  p->kstack = TRAMPOLINE - 2*PGSIZE;
...
int fork(void) {
  ...
  // mapping user space in proc's knl pgtbl as it in user pgtbl
  ...
  for (;;) {
    ...
    for(p = proc; p < &proc[NPROC]; p++) {
      ...
      if (p->state == RUNNABLE) {
        ...
        kvminithart2(p->kernel_pagetable);
...
uint64 proc_num() { return proc_count; }
